...
#---------------- slide days 
for(iii_timeframe in TIME_FRAME){
	...
	#- Features
	Data_train_features <- Data_train[, c(
		"id"                                            
		, "Time"                                                                               
		, "Cases"
		, "Year_original"                                 
		...
		, "TrainTest"                                     
		, "TotalPopulation"                               
		, "TotalPopulation_imp" # equal = TotalPopulation                           
		, "externalEMS_original"
		, "StateID"
		, "Months_imp"
		, Features
		),
	...
	setnames(Data_train_features, c("Cases"), c("Cases_for_check"))
	Data_train_features[, Time_target := Time + iii_timeframe, ]
	Data_train_features[, list(Time, Time_target), ]
	
	
	
	# Outcome
	Data_train_outcome <- Data_train[, c(
		"id"                                            
		, "Time"                                                                               
		, "Cases"                                         
		), 
	...
	setnames(Data_train_outcome, c("Time"), c("Time_target"))
	
	Data_train <- merge(x = Data_train_features, y = Data_train_outcome, by = c("id", "Time_target"))
	write_fst(Data_train, paste0("./input/data_train_for_prediction_model_scaled_slide_", iii_timeframe, "days.fst"))
	...
	#- Features
	Data_test_features <- Data_test[, c(
		"id"                                            
		, "Time"                                                                               
		, "Cases"
		, "Year_original"                                 
	...
	setnames(Data_test_features, c("Cases"), c("Cases_for_check"))
	Data_test_features[, Time_target := Time + iii_timeframe, ]
	Data_test_features[, list(Time, Time_target), ]
	
		
	
	# Outcome
	Data_test_outcome <- Data_test[, c(
		"id"                                            
		, "Time"                                                                               
		, "Cases"                                         
		), 
	...
	setnames(Data_test_outcome, c("Time"), c("Time_target"))
	
	Data_test <- merge(x = Data_test_features, y = Data_test_outcome, by = c("id", "Time_target"))
	write_fst(Data_test, paste0("./input/data_test_for_prediction_model_scaled_slide_", iii_timeframe, "days.fst"))
...
##------------##------------##------------
# Make xgboost model with features selected by ICP with x-day-ahead 
##------------##------------##------------
for(iii_timeframe in TIME_FRAME){
	...
	##------------ Read data
	Data_train <- read_fst(paste0("./input/data_train_for_prediction_model_scaled_slide_", iii_timeframe, "days.fst"), as.data.table = TRUE)
	...
	## CV by manual
	VAL_ALL <- TRUE #TRUE = CV, FALSE = non-CV
	Data_train[, valset := 
	...
	### Save results
	fwrite(Hoge_parameters, paste0("./output/tmp/Table_xgboost_slide_",  iii_timeframe, "days_params_round_r1.csv"))
	...
	### Save results
	fwrite(Hoge_parameters, paste0("./output/tmp/Table_xgboost_slide_",  iii_timeframe, "days_params_round_r2.csv"))
	Next_parameters <- Tune_params[NO == Hoge_parameters[RMSE_val == min(RMSE_val), first(NO), ], , ]
	Next_parameters
	...
	### Save results
	fwrite(Hoge_parameters, paste0("./output/tmp/Table_xgboost_slide_",  iii_timeframe, "days_params_round_r3.csv"))
	Next_parameters <- Tune_params[NO == Hoge_parameters[RMSE_val == min(RMSE_val), first(NO), ], , ]
	Next_parameters
	...
	### Save results
	fwrite(Hoge_parameters, paste0("./output/tmp/Table_xgboost_slide_",  iii_timeframe, "days_params_round_r4.csv"))
	Next_parameters <- Tune_params[NO == Hoge_parameters[RMSE_val == min(RMSE_val), first(NO), ], , ]
	Next_parameters
	...
	### Save results
	fwrite(Hoge_parameters, paste0("./output/tmp/Table_xgboost_slide_",  iii_timeframe, "days_params_round_r5.csv"))
	Next_parameters <- Tune_params[NO == Hoge_parameters[RMSE_val == min(RMSE_val), first(NO), ], , ]
	Next_parameters
	
	
	Next_parameters[, Nround := Hoge_parameters[RMSE_val == min(RMSE_val), Nround, ], ]
	fwrite(Next_parameters, paste0("./output/Table_xgboost_slide_",  iii_timeframe, "days_params_round_best.csv"))
	...
	#----------------#----------------#----------------
	Best_list <- fread(paste0("./output/Table_xgboost_slide_",  iii_timeframe, "days_params_round_best.csv"))
	Best_list
	...
	saveRDS(Model, paste0("./models/XGBoost_slide_",  iii_timeframe, "days_best.rds")) 
	toc()
	...
	importance
	fwrite(importance, paste0("./output/xgboost_slide_",  iii_timeframe, "days_feature_importance_.csv"))
